...
for stage in ['totals']:
    ...
    for datafile_path in to_process:
        ...
        mark_ld_reg = re.compile("ld_([^_]*)_.*")
        no_vms = -1
        ...
        for mark in mk_items:
            m_ld = mark_ld_reg.match(mark['note'])
            if m_ld:
                ld_val = m_ld.group(1)
            else:
                ld_val = -1
            mk_dict[mark['at_id']] = (no_vms, ld_val, mark['note'])

        ex_items = ex_json_data['data']
        (no_vms, ld_val, c_groupex) = mk_dict[0]
        for req_data in ex_items:
            req_dict = {}
            s_dict = {}
            req_id = req_data['id']
            if req_id in mk_dict:
                (no_vms, ld_val, c_groupex) = mk_dict[req_id] # tag point with active marker
            if ld_val == -1:
                continue
            #subsystems
            try:
